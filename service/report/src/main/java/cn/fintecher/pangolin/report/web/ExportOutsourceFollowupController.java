        ...
        try {
            ...
            try {
                ...
                if (!all.isEmpty()) {
                    ...
                    ResponseEntity<String> stringResponseEntity = restTemplate.postForEntity("http://file-service/api/uploadFile/addUploadFileUrl", param, String.class);
                    url = stringResponseEntity.getBody();
                }
            ...
            return ResponseEntity.ok().headers(HeaderUtil.createAlert("开始导出,完成后请前往消息列表查看下载。", "")).body(url);
        } catch (Exception ex) {
