    ...
    public void receive(ImportFileUploadSuccessMessage message) {
        try {
            ...
            ResponseEntity<byte[]> response = restTemplate.exchange(message.getUploadFile().getUrl(),
                    ...
                    byte[].class);
            uploadFileService.uploadCaseFileReduce(new ByteArrayInputStream(response.getBody()), message.getUserId(), message.getUserName(), message.getBatchNum(), message.getCompanyCode(), message.getCaseNumber());
