...
public interface CaseInfoReturnRepository extends QueryDslPredicateExecutor<CaseInfoReturn>, JpaRepository<CaseInfoReturn, String>, QuerydslBinderCustomizer<QCaseInfoReturn> {
    ...
    default void customize(final QuerydslBindings bindings, final QCaseInfoReturn root) {
        ...
        bindings.bind(root.caseId.product.prodcutName).first((path, value) -> path.eq(StringUtils.trim(value)));
        //机构码搜索
        ...
        // 委外回收案件搜索
//        bindings.bind(root.outsourcePool.caseInfo.product.prodcutName).first((path, value) -> path.eq(StringUtils.trim(value)));
//        //机构码搜索
//        bindings.bind(root.outsourcePool.caseInfo.department.code).first((path, value) -> path.startsWith(StringUtils.trim(value)));
//        //公司码
//        bindings.bind(root.outsourcePool.caseInfo.companyCode).first((path, value) -> path.eq(StringUtils.trim(value)));
//        //案件金额
//        bindings.bind(root.outsourcePool.caseInfo.overdueAmount).all((path, value) -> {
//            Iterator<? extends BigDecimal> it = value.iterator();
//            BigDecimal firstOverdueAmount = it.next();
//            if (it.hasNext()) {
//                BigDecimal secondOverDueAmont = it.next();
...
//        });
//        //逾期天数
//        bindings.bind(root.outsourcePool.caseInfo.overdueDays).all((path, value) -> {
//            Iterator<? extends Integer> it = value.iterator();
//            Integer firstOverdueDays = it.next();
//            if (it.hasNext()) {
//                Integer secondOverdueDays = it.next();
...
//        });
//        //案件手数
//        bindings.bind(root.outsourcePool.caseInfo.handNumber).all((path, value) -> {
//            Iterator<? extends Integer> it = value.iterator();
//            Integer firstHandNumber = it.next();
//            if (it.hasNext()) {
//                Integer secondHandNumber = it.next();
...
//        });
//        //佣金比例%
//        bindings.bind(root.outsourcePool.caseInfo.commissionRate).all((path, value) -> {
//            Iterator<? extends BigDecimal> it = value.iterator();
//            BigDecimal firstCommissionRate = it.next();
//            if (it.hasNext()) {
//                BigDecimal secondCommissionRate = it.next();
...
//        });
//        //委案日期
//        bindings.bind(root.outsourcePool.caseInfo.delegationDate).all((path, value) -> {
//            Iterator<? extends Date> it = value.iterator();
//            Date firstDelegationDate = it.next();
//            if (it.hasNext()) {
//                Date secondDelegationDate = it.next();
...
//        });
//        //结案日期
//        bindings.bind(root.outsourcePool.caseInfo.closeDate).all((path, value) -> {
//            Iterator<? extends Date> it = value.iterator();
//            Date firstCloseDate = it.next();
//            if (it.hasNext()) {
//                Date secondCloseDate = it.next();
...
//        });
//        //委托方
//        bindings.bind(root.outsourcePool.caseInfo.principalId.id).first((path, value) -> path.eq(StringUtils.trim(value)));
//        //案件状态
//        bindings.bind(root.outsourcePool.caseInfo.collectionStatus).first((path, value) -> path.eq(value));
//        //案件类型
//        bindings.bind(root.outsourcePool.caseInfo.caseType).first((path, value) -> path.eq(value));
//        //催收类型
//        bindings.bind(root.outsourcePool.caseInfo.collectionType).first((path, value) -> path.eq(value));
//        //产品系列
//        //bindings.bind(root.caseInfo.product.productSeries.id).first((path, value) -> path.eq(StringUtils.trim(value)));
//        //客户姓名
//        bindings.bind(root.caseId.personalInfo.name).first((path, value) -> path.contains(StringUtils.trim(value)));
//        //客户手机号
//        bindings.bind(root.outsourcePool.caseInfo.personalInfo.mobileNo).first((path, value) -> path.eq(StringUtils.trim(value))
//                .or(root.outsourcePool.caseInfo.personalInfo.personalContacts.any().phone.eq(StringUtils.trim(value))));
//        //批次号
//        bindings.bind(root.outsourcePool.caseInfo.batchNumber).first((path, value) -> path.eq(StringUtils.trim(value)));
//        //申请省份
//        //bindings.bind(root.caseInfo.area.parent.id).first((path, value) -> path.eq(value));
//        //申请城市
//        bindings.bind(root.outsourcePool.caseInfo.area.id).first((path, value) -> path.eq(value));
//        //还款状态
...
//        list1.add("M4");
//        list1.add("M5");
//        bindings.bind(root.outsourcePool.caseInfo.payStatus).first((path, value) -> {
//            if (Objects.equals(StringUtils.trim(value), CaseInfo.PayStatus.M6_PLUS.getRemark())) {
//                return path.notIn(list1);
//            } else {
//                return path.eq(value);
//            }
//        });
